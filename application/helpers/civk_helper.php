...
if ( ! function_exists('parse_search_string'))
{
	/**
	 * Parse Search String
	 * @param	string
  ...
  function parse_search_string(&$objects, $search_str = '')
  {
    ...
    for($k = 0; $k < count($pieces); $k++)
    {
      ...
      {
        $tmp_string = trim(preg_replace('/"/', ' ', $pieces[$k]));
...
if ( ! function_exists('format_timestamp'))
{
  ...
  function format_timestamp($from, $to = false, $str = true)
  {
    $from_date = new DateTime(date('Y-m-d H:i:s', strtotime($from)));
    if(!$to)
    {
      $to_date = new DateTime(date('Y-m-d H:i:s'));
    }
    else
    {
    ...
    if($interval->invert)
    {
      if(($interval->y > 1))
      {
        $timestamp = $interval->y.' years ago';
      }
      elseif(($interval->y > 0))
      {
        $timestamp = $interval->y.' year ago';
      }
      elseif ($interval->m > 1)
      {
        $timestamp = $interval->m.' months ago';
      }
      elseif ($interval->m > 0)
      {
        $timestamp = $interval->m.' month ago';
      }
      elseif ($interval->d > 1)
      {
        $timestamp = $interval->d.' days ago';
      }
      elseif ($interval->d == 1)
      {
        $timestamp = $interval->d.' day ago';
      }
      elseif ($interval->h > 1)
      {
        $timestamp = $interval->h.' hours ago';
      }
      elseif ($interval->i > 1)
      {
        $timestamp = $interval->i.' mins ago';
      }
       elseif ($interval->i > 0)
      {
        $timestamp = $interval->i.' min ago';
      }
      else
      {
        $timestamp = 'Just Now';
...
if ( ! function_exists('create_random_value'))
{
   ...
   *        $type string //mixed, chars, digits
   * @return string
   */
  ...
  function create_random_value($length, $type = "mixed")
  {
    ...
    while(strlen($randomValue) < $length)
    {
      if($type == "digits")
      {
        $char = random_value(0, 9);
      }
      else
      {
        $char = chr(random_value(0, 255));
      }
...
if ( ! function_exists('random_value'))
{
  ...
  function random_value($min = NULL, $max = NULL)
  {
    static $seeded;
...
if ( ! function_exists('format_date_time'))
{
  ...
  function format_date_time($date, $format = 'M d, Y h:i A')
  {
    $date = new DateTime($date);
...
// ------------------------------------------------------------------------
   ...
   * @return  string
   */
  function site_url_vk($uri = '', $query_string = array())
  {
    $CI =& get_instance();
    ...
    if (!empty($uri))
    {
      $base_url .= $CI->config->item('index_page');
    }
    ...
    if ($query_string)
    {
      $http_build_query = http_build_query($query_string);
    ...
    return $base_url.$CI->config->item('index_page').$uri.$http_query;
  }

// ------------------------------------------------------------------------


  function format_number($number, $symbol = false, $decimals = 2, $dec_point ='.', $thousands_sep = '')
  {
    if($symbol)
    {
  ...
  //Functions for database fields encryption and decryption

  function dbAesEncrypt($simple_text)
  {
    $CI =& get_instance();
    $text = $CI->db->escape($simple_text);
    $sql_part = "TO_BASE64(AES_ENCRYPT(".$text.", '".$CI->config->item('encryption_key')."'))";
    return $sql_part;
  }

  function dbAesDecrypt($field_name)
  {
    $CI =& get_instance();
    $sql_part = "AES_DECRYPT(FROM_BASE64(".$field_name."), '".$CI->config->item('encryption_key')."')";
    return $sql_part;
  }

  function dbAesDecryptSpecial($field_name)
  {
    $CI =& get_instance();
    $sql_part = "AES_DECRYPT(FROM_BASE64(`".$field_name."`), '".$CI->config->item('encryption_key')."')";
    return $sql_part;
  ...
  function timeToRoundOff($date_time)
  {
    /*
      Conditions
      7:00 To 7:07 => 7:00
    ...
    */
    ...
    $hrs  = date('H', strtotime($date_time));
    $mins = date('i', strtotime($date_time));

    $result_time = $date_time;
    if(($mins >= 0) && ($mins <= 7))
    {
      $result_time = $hrs.':00:00';
    }
    else if(($mins >= 8) && ($mins <= 22))
    {
      $result_time = $hrs.':15:00';
    }
    else if(($mins >= 23) && ($mins <= 37))
    { 
      $result_time = $hrs.':30:00';
    }
    else if(($mins >= 38) && ($mins <= 52))
    { 
      $result_time = $hrs.':45:00';
    }
    else if($mins >= 53)
    { 
      if($hrs == 23)
  ...
  function format_currency($number, $symbol = true, $decimals = 2, $dec_point ='.', $thousands_sep = ',')
  {
    ...
    if($number < 0)
    {
      return '-' . ($symbol ? $currency_symbol : '') . number_format(abs($number), $decimals, $dec_point, $thousands_sep);
    }
    else
    {
  ...
  function date_diffrence($datefrom, $dateto, $str=true)
  {
    $createdDate = new DateTime(date('Y-m-d H:i:s', strtotime($datefrom)));
    $expiredDate = new DateTime(date('Y-m-d H:i:s', strtotime($dateto)));
    ...
    return array('class' => $class, 'timer' => $interval->d . 'd ' . ($interval->h) .'h ' . $interval->i.'m');
  }

  function format_timestamp($from, $to, $str = true)
  {
    $from_date = new DateTime(date('Y-m-d H:i:s', strtotime($from)));
    $to_date = new DateTime(date('Y-m-d H:i:s', strtotime($to)));
    $interval = $to_date->diff($from_date);
    ...
    if($interval->invert)
    {
      if(($interval->y > 0))
      {
        $timestamp = date('F j, Y', strtotime($from)).' at '.date('h:ia', strtotime($from));
      }
      elseif ($interval->m > 0)
      {
        $timestamp = date('F j', strtotime($from)).' at '.date('h:ia', strtotime($from));
      }
      elseif ($interval->d > 1)
      {
        $timestamp = date('F j', strtotime($from)).' at '.date('h:ia', strtotime($from));
      ...
      elseif ($interval->d == 1)
      {
        $timestamp = 'Yesterday at '.date('h:ia', strtotime($from));
      }
      elseif ($interval->h > 1)
      {
        $timestamp = date('h:ia', strtotime($from));
      }
      elseif ($interval->i > 0)
      {
        $timestamp = $interval->i.' Min';
      }
      else
      {
        $timestamp = 'Just Now';
  ...
  function num_days($day, $month, $year)
  {
    ...
    {
      echo "num_days: invalid argument. \$day must be day name or three letter abbreviation";
      return;
    }
    ...
    {
      echo "num_days: invalid argument. \$month must be month name or three letter abbreviation";
      return;
    }
  ...
  function strPlural($amount, $singular = '', $plural = 's' ) {
    if ($amount > 1) {
      return $plural;
    }
    return $singular;
